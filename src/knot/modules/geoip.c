...
static int get_query_source(struct qsource *src, knot_pkt_t *pkt, struct query_data *qdata)
{
	...
	int r = knot_edns_get_option(&qdata->opt_rr, KNOT_EDNS_OPTION_CLIENT_SUBNET,
	                             &src->ecs_wire, &src->ecs_size);
	if (r != KNOT_EOK) {
		src->addr = *qdata->param->remote;
		return KNOT_EOK;
	}

	r = knot_edns_client_subnet_parse(&src->ecs_data, src->ecs_wire, src->ecs_size);
	if (r != KNOT_EOK) {
		return r;
	}
