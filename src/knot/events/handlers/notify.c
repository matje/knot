...
static int notify_produce(knot_layer_t *layer, knot_pkt_t *pkt)
{
	...
	// voluntary: NOTIFY opcode, AA flag, SOA qtype
	query_init_pkt(pkt);
                       ...
                       const conf_remote_t *slave, int timeout, uint16_t *rcode)
{
	...
	struct knot_request *req = knot_request_make2(NULL, dst, src, pkt, &slave->key, 0);
	if (!req) {
...
#define NOTIFY_LOG(priority, zone, remote, msg, ...) \
	NS_PROC_LOG(priority, zone, remote, "NOTIFY, outgoing", msg, ##__VA_ARGS__)

static void log_notify_result(int ret, uint16_t rcode, const knot_dname_t *zone,
                              const struct sockaddr_storage *remote, uint32_t serial)
{
	if (ret == KNOT_EOK) {
...
int event_notify(conf_t *conf, zone_t *zone)
{
	...
	// NOTIFY content
	int timeout = conf_int(&conf->cache.srv_tcp_reply_timeout) * 1000;
